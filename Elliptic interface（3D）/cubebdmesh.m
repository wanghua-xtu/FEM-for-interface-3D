function  T = cubebdmesh(box,h,example,r)
...
%
% Example
%box=[-3,3,-3,3,-3,3]; h = 1/8; example = 3; r =0.67777;
%box=[-1,1,-1,1,-1,1]; h = 1/16; example = 1; r =0.67777;
...
for i = 1:length(cutcase1)
    idxi = sum(cm(cutelem1id,:).*transmatrix(cutelem1num,:),2)==cutcase1(i);
    j = j0+2*sum(idxi)-1;
...
for i = 1:length(cutcase2)
    idxi = sum(cm(cutelem2id,:).*transmatrix(cutelem2num,:),2)==cutcase2(i);
    j = j0+sum(idxi)-1;
...
for i = 1:length(cutcase3)
    idxi = sum(cm(cutelem3id,:).*transmatrix(cutelem3num,:),2)==cutcase3(i);
    j = j0+sum(idxi)-1;
...
for i = 1:length(cutcase4)
    idxi = sum(cm(cutelem4id,:).*transmatrix(cutelem4num,:),2)==cutcase4(i);
    j = j0+2*sum(idxi)-1;
...
%% genrate new element data
% elem , sign of element, interface(face in interface) 

% isCutElem  = (eta < 0 | sum(abs(e2vSign),2) <= 1); % elements cross interface
...
tetelem = tetelem(tetsign<=0,:);
prielem = prielem(prisign<=0,:);
pyrelem = pyrelem(pyrsign<=0,:);

T = struct('node',node,'tetelem',tetelem,'pyrelem',pyrelem, 'prielem',prielem, ...
          'vsign',vSign,'tetsign',tetsign,'pyrsign',pyrsign,'prisign',prisign);
